...
public class BlokusTwoPeopleController : MonoBehaviour {
    ...
    void changeCurrentColor() {
        CurrentColor++;
        if (CurrentColor > maxPlayerCount) {
            CurrentColor = 1;
        }

        while (loseColor[CurrentColor] == maxPlayerCount) {
            CurrentColor++;
            if (CurrentColor > maxPlayerCount) {
                CurrentColor = 1;
            }
    ...
    {
        if (x < 0 || x > MAX_LINE - 1 || y < 0 || y > MAX_LINE - 1) {
            return true;
        }
        return false;
    }
